...
def test_assert_with_list_as_external_id_succeeds(yaml_with_sf_object_having_list_as_external_id):
    mock_sf_connection = MagicMock()

    with patch("builtins.open", mock_open(read_data=yaml_with_sf_object_having_list_as_external_id)), \
         patch('sattl.salesforce.object.SalesforceObject.differences', return_value=None) as mock_so_differences:
        test_assert = TestAssert("00-assert.yaml", sf_connection=mock_sf_connection)
        test_assert.validate()
    ...
    mock_so_differences.assert_has_calls([
        call(SalesforceObject(mock_sf_connection, dict(externalID=dict(Slug__c="aaa1"), **data))),
        call(SalesforceObject(mock_sf_connection, dict(externalID=dict(Slug__c="aaa2"), **data))),
        ]
    )
...
def test_assert_fails_because_cannot_load_it(yaml_with_five_sf_objects, sample_object_content):
    with patch("builtins.open", mock_open(read_data=yaml_with_five_sf_objects)), \
         patch('sattl.salesforce.object.SalesforceObject.load', return_value=False) as mock_so_differences, \
